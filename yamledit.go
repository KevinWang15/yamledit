...
func indexSequenceAnchors(st *docState, seq *yaml.Node, cur []string) {
	...
	computeItemBounds := func(it *yaml.Node) (start int, end int, name string) {
		...
		if it.Kind == yaml.MappingNode {
			// IMPORTANT: use maxLineEndForNode so we include nested structures,
			// e.g. a mapping that contains:
			//   paths:
			//     - /api
			//     - /v1
			//
			// This ensures the item boundary extends through the final "- /v1" line.
			maxEnd := 0
			for j := 0; j+1 < len(it.Content); j += 2 {
				v := it.Content[j+1]
				if v == nil {
		...
		// Detect style/indent/key order once, using the first item.
		if !detectedStyle {
			...
			// indent for "- ..." line and whether first key is inline.
			si.indent = leadingSpaces(ln)
			si.firstKeyInline = firstNonSpaceByte(ln) == '-' // True if "- key: value" style
